-- Application schema, generation 2.
--
-- Conventions:
--   * every timestamp is INTEGER milliseconds since the Unix epoch, set by
--     the Rust side;
...
-- normalised. Kept as real columns so it can be queried, with '' rather than
-- NULL for the WSL distro so the UNIQUE constraint actually holds.
------------------------------------------------------------------------------
...
-- the log cannot express. Always written in the same transaction as the log.
...
-- UI state
--
-- A single-row table. The row is not seeded here: the reader falls back to
-- defaults when it is absent and the writer upserts, which also keeps a
-- recovered row from being shadowed by a seed.
------------------------------------------------------------------------------
