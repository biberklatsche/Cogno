...
const unitGenerator: Generator = {
    custom: async (tokens, executeShellCommand) => {
        ...
        const suggustions = units.map((unit) => {
            ...
            const activeString = unit.active.charAt(0).toUpperCase() + unit.active.slice(1);
            return {
                name: unit.unit,
        ...
        suggustions.sort((a, b) => a.name.localeCompare(b.name, undefined, { usage: "sort" }));
        return suggustions;
    },
};
type UnitFile = {
...
const unitFileGenerator: Generator = {
    custom: async (tokens, executeShellCommand) => {
        ...
        const suggustions = units.map((unit) => {
            ...
            const loadedString = unit.state?.charAt(0).toUpperCase() + unit.state?.slice(1);
            return {
                name: unit.unit_file,
