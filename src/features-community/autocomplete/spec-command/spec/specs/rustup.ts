...
const toolchainLocalGenertor: (args?: ToolchainLocalGeneratorOptions) => Generator = ({ excludeShort } = {}) => ({
    ...
    postProcess: (out) => {
        ...
        if (!excludeShort) {
            ...
            // dedupe
            shortToolchains = shortToolchains.filter((toolchain, index) => shortToolchains.indexOf(toolchain) === index);
        }
    ...
    postProcess: (out) => {
        ...
        return [
            ...
            ...releases.map((release) => ({
                ...
                description: `${release.name} - ${new Date(release.published_at).toLocaleDateString()}`,
            }))
        ];
    },
};
...
const tripleGenerator: (args?: TripleGeneratorOptions) => Generator = ({ installed, } = {}) => ({
