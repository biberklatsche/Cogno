...
function flattenSubcommands(separator: string, parents: TwilioSubcommand[], subcommand: TwilioSubcommand): TwilioSubcommand[] {
    // If this subcommand has children, flatten it further.
    if (subcommand.subcommands && subcommand.subcommands.length > 0) {
        return subcommand.subcommands.flatMap((child) => flattenSubcommands(separator, parents.concat(subcommand), child));
    }
 ...
 * and separator.
 */
function createSeparatedSubcommandTree(separator: string, subcommands: TwilioSubcommand[], options: {
    generatorShouldInsertSeparator: boolean;
}): {
    subcommands?: TwilioSubcommand[];
    args: ArgSpec;
} {
    const flattened = subcommands.flatMap((subcommand) => flattenSubcommands(separator, [], subcommand));
...
const emailSubcommand: TwilioSubcommand = {
    name: "email",
    description: "Send emails to single or multiple recipients using Twilio SendGrid",
    subcommands: [
        {
            name: "send",
