...
const resolveAbsolutePath = (path: string, basePath: string, home: string): string => {
...
const getConfigLines = async (file: string, executeShellCommand: ExecuteCommandFunction, home: string, basePath: string) => {
    ...
    const includeLines: string[][] = await Promise.all(includes.map((file) => getConfigLines(file, executeShellCommand, home, basePath)));
