...
const fillProjectCaches = (projectJson: NxProject) => {
    if (!nxProjectWithTargetsCache.has(projectJson.name)) {
        ...
        for (const target in projectJson.targets) {
            ...
            nxProjectTargetWithConfigurationsCache.set(`${projectJson.name}:${target}`, projectTargetConfigurations);
        }
...
const preProcessProjects = async (executeShellCommand: ExecuteCommandFunction) => {
    if (!nxProjectPathCache.length) {
        // get project json paths
        try {
            const { appsDir, libsDir }: {
                appsDir: string;
                libsDir: string;
            } = {
                appsDir: "apps",
                libsDir: "libs",
                ...JSON.parse((await executeShellCommand({
                    command: "cat",
                    // eslint-disable-next-line @withfig/fig-linter/no-useless-arrays
                    args: ["nx.json"],
                })).stdout).workspaceLayout,
            };
            const searchFolders = appsDir === libsDir ? [appsDir] : [appsDir, libsDir];
            nxProjectPathCache = (await executeShellCommand({
                command: "find",
    ...
    for (const projectJsonPath of nxProjectPathCache) {
        ...
        if (!projectJson) {
            try {
                projectJson = JSON.parse((await executeShellCommand({
                    command: "cat",
                    // eslint-disable-next-line @withfig/fig-linter/no-useless-arrays
    ...
    if (!nxProjectPathCache.length) {
        try {
            nxWorkspaceJsonCache = JSON.parse((await executeShellCommand({
                command: "cat",
                // eslint-disable-next-line @withfig/fig-linter/no-useless-arrays
                args: ["workspace.json"],
            })).stdout);
...
const listMapKeysGenerator = (map: Map<string, unknown>): Generator => {
    return {
        cache: oneDayCache,
        trigger: (newToken, oldToken) => newToken.split(",").length !== oldToken.split(",").length,
        getQueryTerm: (token) => token.split(",").pop(),
        custom: async (tokens: string[], executeShellCommand: ExecuteCommandFunction, generatorContext: GeneratorContext) => {
...
const nxGenerators: NxGenerators = {
    configuration: {
        cache: oneDayCache,
        custom: async (tokens: string[], executeShellCommand: ExecuteCommandFunction) => {
            const suggestions: Suggestion[] = [];
            const finalToken = [];
            ...
            for (const name of nxProjectTargetWithConfigurationsCache.get(`${project}:${target}`)) {
    ...
    project: {
        ...
        custom: async (_: string[], executeShellCommand: ExecuteCommandFunction) => {
        ...
        custom: async (tokens: string[], executeShellCommand: ExecuteCommandFunction, generatorContext: GeneratorContext) => {
            ...
            if (!project) {
                ...
                for (const name of nxProjectWithTargetsCache.get(project)) {
                    if (nxProjectTargetWithConfigurationsCache.get(`${project}:${name}`)) {
                        suggestions.push({ name, insertValue: `${name}:` });
                    }
                    else {
                        suggestions.push({ name });
 ...
 */
const runDerivedSubcommands = async (_: string[], executeShellCommand: ExecuteCommandFunction): Promise<CommandSpec> => {
