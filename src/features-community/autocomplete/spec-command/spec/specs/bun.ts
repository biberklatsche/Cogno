...
import { filepaths, keyValueList, valueList } from "@fig/autocomplete-generators";
import { npmScriptsGenerator, npmSearchGenerator, dependenciesGenerator, } from "./npm";
...
const inspectArgs: SingleOrArray<ArgSpec> = {
    name: "[host:]port",
...
const sharedPublicParams: OptionSpec[] = [
    {
        name: ["-h", "--help"],
...
const notBunDevFlags: OptionSpec[] = [
    {
        name: "--hot",
...
const buildOnlyParams: OptionSpec[] = [
    {
        name: "compile",
...
const testOnlyParams: OptionSpec[] = [
    {
        name: "--timeout",
...
const dependencyOptions: OptionSpec[] = [
    {
        name: ["-c", "--config"],
        args: {
            name: "path"
...
const bunLinksGenerator: Generator = {
    script: [
        "command",
        "sh",
        "-c",
...
const spec: CommandSpec = {
    ...
    options: publicParams.filter((param) => param.name.includes("--inspect") ||
