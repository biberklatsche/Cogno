...
const uniqBy = <T = unknown>(arr: T[], callback: (a: T, b: T) => boolean) => arr.reduce((acc, v) => {
    if (!acc.some((x) => callback(v, x)))
        acc.push(v);
    return acc;
}, []);
/**
...
const remoteVersionGenerator: Generator = {
    ...
    postProcess: function (out) {
        ...
        const nodeVersion = parsed
            ...
            return ((version.major === lastLtsMajor - 1 ||
                version.major === lastLtsMajor + 1) &&
                isLatestRelease);
        });
        // Uniquify the list, and put them in a Suggestion object.
        return uniqBy([...nodeVersion, ...parsed], (a, b) => a.original === b.original).map((version) => version.ltsName &&
