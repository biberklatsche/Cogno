...
export class NpmScriptsSpecProvider implements SpecSuggestionProvider {
    ...
    async suggest(context: SpecProviderContext): Promise<ReadonlyArray<SpecProvidedSuggestion>> {
        ...
        try {
            if (!(await this.filesystem.exists(packageJsonPath, context.queryContext.shellContext))) return [];
            const text = await this.filesystem.readTextFile(packageJsonPath, context.queryContext.shellContext);
