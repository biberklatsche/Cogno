...
export class KeybindExecutor implements IDisposable {
      ...
      this._bus
        ...
        .subscribe(async (event) => {
          ...
          switch (event.payload) {
            ...
            case "copy": {
              const isPerformable = this.calcPreventDefault(
                event.trigger?.performable,
                () => this._stateManager.hasSelection,
              );
              if (isPerformable) {
                this._bus.publish({
                  type: "Copy",
            ...
            case "go_to_start_of_line": {
              this._bus.publish({
                type: "GoToStartOfLine",
                payload: this._stateManager.terminalId,
                path: ["app", "terminal"],
              });
              event.performed = true;
  ...
  private calcPreventDefault(
    performableTrigger: boolean | undefined,
    proof: () => boolean,
  ): boolean {
