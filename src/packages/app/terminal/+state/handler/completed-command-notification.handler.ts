...
export class CompletedCommandNotificationHandler {
    ...
    initialize(): void {
        this.longRunningCommandNotificationsEnabled = this.getDefaultLongRunningCommandNotificationsEnabled();
    }
    ...
    readonly handleCompletedCommand = (executedCommand: ExecutedCommand): void => {
        ...
        if (commandDurationMilliseconds === undefined || commandDurationMilliseconds < minimumDurationMilliseconds) {
            return;
        }
    ...
    private getLongRunningCommandMinimumDurationMilliseconds(): number {
        const minimumDurationSeconds = this.configService.config.notification?.long_running_commands?.minimum_duration_seconds ?? 10;
