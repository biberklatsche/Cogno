...
export class CommandPatternLearner {
    ...
    updatePattern(
        ...
        const slotValuesByIndex = new Map(
            commandPatternOccurrence.slotValues.map((slotValue) => [slotValue.slotIndex, slotValue.value]),
        );

        const slotAccumulators = existingPatternAccumulator?.slotAccumulators.map((slotAccumulator) => ({
            ...slotAccumulator,
    ...
    toLearnedPattern(patternAccumulator: PatternAccumulator): LearnedCommandPattern {
        const stableTokenCount = patternAccumulator.signature.parts.filter((part) => part.kind === "stable").length;
        const nonOptionStableTokenCount = patternAccumulator.signature.parts.filter(
            (part) => part.kind === "stable" && !part.value.startsWith("-"),
        ).length;
        const variableSlotCount = patternAccumulator.signature.parts.filter((part) => part.kind === "slot").length;
    ...
    private createSlotAccumulators(signatureParts: CommandSignaturePart[]): SlotAccumulator[] {
        return signatureParts.flatMap((signaturePart) => signaturePart.kind === "slot"
            ? [{
