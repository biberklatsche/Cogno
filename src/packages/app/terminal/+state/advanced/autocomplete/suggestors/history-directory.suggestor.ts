...
export class HistoryDirectorySuggestor implements TerminalAutocompleteSuggestor {
  ...
  private async suggestDirectories(context: CdQueryContext): Promise<AutocompleteSuggestion[]> {
    ...
    for (const row of rows) {
      const effectiveTokens =
        tokens.length > 0 ? tokens : this.extractTokens(context.fragment, context.shellContext);
      ...
      if (
        ...
        continue;
      const directoryPath = AutocompletePathSupport.appendDirectorySeparator(
        displayPath,
  ...
  private extractTokens(fragment: string, shellContext: CdQueryContext["shellContext"]): string[] {
    return AutocompletePathSupport.splitAutocompleteFragmentTokens(fragment, shellContext)
