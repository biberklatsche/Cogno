...
export class OllamaNativeProviderAdapter implements LlmProviderAdapter {
  ...
  async *streamChat(
    ...
    if (!response.body) {
      const fallbackResponse = (await response.json()) as OllamaStreamChunk;
    ...
    while (true) {
      ...
      for (const line of lines) {
        ...
        const parsedChunk = JSON.parse(trimmedLine) as OllamaStreamChunk;
    ...
    if (pendingText.trim().length > 0) {
      const parsedChunk = JSON.parse(pendingText.trim()) as OllamaStreamChunk;
      const chunkText = parsedChunk.message?.content ?? "";
      if (chunkText) {
        yield { text: chunkText };
      }
