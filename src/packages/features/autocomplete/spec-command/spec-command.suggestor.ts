...
export class SpecCommandSuggestor implements TerminalAutocompleteSuggestorContract {
  ...
  matches(context: AutocompleteQueryContextContract): boolean {
    return (
      (context.mode === "command" || context.mode === "cd") &&
  ...
  async suggest(
    ...
    if (parsed.activeTokenIndex === 0) {
      return this.suggestCommandNames(
        parsed.activeValue,
        parsed.activeStart,
        parsed.activeEnd,
        context,
  ...
  private async suggestCommandArgs(
    ...
    if (traverse.pendingArgs > 0 && traverse.pendingArgSource === "subcommand") {
      ...
      for (const option of optionMap.values()) {
        ...
        const allUsed = aliases.every((alias) =>
          traverse.usedOptionAliases.has(alias.toLowerCase()),
        );
  ...
  private traverse(
    root: PreparedNode,
    committedTokens: Array<{ value: string; start: number; end: number }>,
  ): TraverseState {
    ...
    for (const token of committedTokens) {
      ...
      const matchedOption =
        state.node.optionByAlias.get(tokenLower) ?? root.optionByAlias.get(tokenLower);
      if (matchedOption) {
  ...
  private pickSuggestionName(names: string[], activeToken: string): string {
    ...
    return (
      names.find((n) => n.toLowerCase().startsWith(lowered)) ??
      names.find((n) => n.toLowerCase().includes(lowered)) ??
      names[0]
    );
  }

  private matchesProviderBinding(
    binding: SpecProviderBinding,
    parsed: ParsedInput,
    argsInput: string,
  ): boolean {
  ...
  private commandsForShell(
    context: AutocompleteQueryContextContract,
  ): Promise<ShellScopedCommand[]> {
  ...
  private commandsForShellType(shellType: ShellConstraint): Promise<ShellScopedCommand[]> {
    ...
    const next = Promise.resolve(this.registry.commandNames()).then(async (names) => {
