...
describe("CommandSpecRegistry", () => {
  ...
  it("keeps provider bindings from specs", () => {
    ...
    const run = (npm?.subcommands as any[]).find((v) =>
      typeof v === "string" ? v === "run" : v.name === "run",
    );
    ...
    const yarnRun = (yarn?.subcommands as any[]).find((v) =>
      typeof v === "string" ? v === "run" : v.name === "run",
    );
    ...
    const pnpmRun = (pnpm?.subcommands as any[]).find((v) =>
      typeof v === "string" ? v === "run" : v.name === "run",
    );
