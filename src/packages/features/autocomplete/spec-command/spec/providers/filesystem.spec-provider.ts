...
export class FilesystemSpecProvider implements SpecSuggestionProvider {
  ...
  private resolveKinds(params: FilesystemSpecProviderParams): FilesystemProviderKind[] {
    ...
    const filtered = kinds.filter(
      (kind): kind is FilesystemProviderKind => kind === "file" || kind === "directory",
    );
  ...
  private resolveLookup(
    ...
    const normalizedFragment =
      shellContext.shellType === "PowerShell"
  ...
  private async readCandidates(
    ...
    for (const entry of entries) {
      ...
      const insertText =
        entry.kind === "directory" && appendSlashToDirectories
      ...
      candidates.push({
        ...
        suggestion: {
          label: AutocompletePathSupport.shortenParentTraversalDisplay(insertText, shellContext),
          insertText: escapedInsertText,
          selectedPath: entry.path,
          completionBehavior:
            continueSuggestions && entry.kind === "directory" ? "continue" : "final",
        },
      });
    }
